    ...
    return (
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-sm text-center max-w-md w-full">
          ...
          <p className="text-gray-500 mb-6">Select up to 4 properties from the listings page to see them side-by-side.</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight">Compare Properties</h1>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="overflow-x-auto scrollbar-hide">
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr>
                  <th className="p-6 bg-gray-50 w-48 border-b border-gray-200 sticky left-0 z-10 shadow-[1px_0_0_0_#e5e7eb]">
                    <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Features</span>
                  </th>
                  {properties.map(property => (
                    <th key={property.id} className="p-6 border-b border-gray-200 border-l min-w-[280px] align-top relative bg-white">
                      <button 
                        onClick={() => removeFromCompare(property.id)}
                        className="absolute top-4 right-4 p-1.5 bg-gray-100 text-gray-500 hover:bg-red-50 hover:text-red-600 rounded-full transition-colors"
                        title="Remove"
                      >
                        <X className="w-4 h-4" />
                      </button>
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {features.map((feature, idx) => (
                  <tr key={feature.key} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="p-4 font-medium text-gray-900 border-l border-r border-gray-200 sticky left-0 shadow-[1px_0_0_0_#e5e7eb] bg-inherit">
                      {feature.label}
                    </td>
                    {properties.map(property => (
                      <td key={property.id} className="p-4 text-gray-600 border-r border-gray-200">
                ...
                <tr>
                  <td className="p-4 font-medium text-gray-900 border-l border-r border-gray-200 sticky left-0 shadow-[1px_0_0_0_#e5e7eb] bg-white">
                    Description
                  </td>
                  {properties.map(property => (
                    <td key={property.id} className="p-4 text-gray-500 border-r border-gray-200 align-top">
